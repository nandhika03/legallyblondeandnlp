# NLP through Legally Blonde

_This project is intended only for learning purposes_

#### What's in here?

1. Streamlit application with basic NLP topics with examples
2. Bag of Words, N-Grams
3. TF-IDF, Word2Vec, avgwordtovec

You can find the movie script of legally blonde [here](https://github.com/nandhika03/legallyblondeandnlp/blob/main/data/Script_LegallyBlonde.txt)

![image](https://y.yarn.co/9b3fed8f-30f4-4b65-8d93-3114372519e9_text.gif)

#### look at the wordcloud for our beloved Emmett

![image](C://Users//Nandhika//Desktop//LegallyBlonde//data//warner_bagofwords.png)
FYI,

- A WordCloud is a visual representation of text data
- Word size = frequency or importance of that word in the corpus
- Usually ignores stopwords (like “the”, “and”, etc.)
